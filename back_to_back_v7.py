...
# Gera o gráfico
fig = plot_inrush(t, i_curto, i_pico_inicial, omega, sigma, f_fund, text, X_curto_circuito, X, I_fn)
...
st.write(text["peak_current_energization"], format_input(i_pico_inicial_isolado, language_key), "${\\rm A}$   (",
         format_input(np.round(corrente_pico_bancos_isolado, 1), language_key), "$\\times I_{\\rm{rated}}$)")
st.write(text["oscillation_frequency"], format_input(w_isolado / (2 * np.pi), language_key), "${\\rm Hz}$   (",
         format_input(np.round(w_isolado / w_fund, 1), language_key), "$\\times f_1$)")

st.markdown(text["for_bank_with_others_energized"])
corrente_pico_bancos_back_to_back = i_pico_inicial / (I_fn * np.sqrt(2))
st.write(text["peak_current_energization"], format_input(i_pico_inicial, language_key), "${\\rm A}$   (",
         format_input(np.round(corrente_pico_bancos_back_to_back.max(), 1), language_key), "$\\times I_{\\rm{rated}}$)")
freq_oscilacao = omega / (2 * np.pi)
st.write(text["oscillation_frequency"], str(int(freq_oscilacao)), "${\\rm Hz}$   (",
         format_input(np.round(omega / w_fund, 1), language_key), "$\\times f_1$)")
