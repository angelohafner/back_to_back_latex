...
\begin{itemize}[label=\textendash]
	\item	Potencia reativa  = 27.0 MVAr
	\item	Tensão trifásica  = 13.8 kV
	\item	Tensão monofásica  = 8.0 kV
	\item	Corrente de curto-circuito  = 20.0 kA
\end{itemize}
...
São os valores obtidos com o reator escolhido ($L_{reator} = 15.0 \, \mu \rm{H} $):
\begin{itemize}[label=\textendash]
	\item	Corrente de pico: 24.5 kA;
	\item	Frequência de Oscilação: 4.1 kHz;
	\item	Corrente inrush / Corrente nominal: 76.8
\end{itemize}

\section{Conclusão}
Geeigneter Reaktor, da $\dfrac{I_{\rm inrush}}{I_{\rm nominal}} = $76.83$\le 100$ e $f_{osc} = $4.09kHz < 4,25 kHz, IEEE Std C37.012, p. 16[$^{[2]}$](https://ieeexplore.ieee.org/document/7035261) e IEC 62271-100, Table 9 (Preferred values of rated capacitive switching currents), p. 45[$^{[3]}$](https://webstore.iec.ch/publication/62785).
